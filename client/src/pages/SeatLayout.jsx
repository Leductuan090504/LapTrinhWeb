...
const SeatLayout = () => {
  ...
  const handleSeatClick = (seatId) =>{
      ...
      if(!selectedSeats.includes(seatId) && selectedSeats.length > 4){
        return toast("Bạn chỉ có thể chọn 5 chỗ ngồi.")
      }
      if(occupiedSeats.includes(seatId)){
        return toast('Chỗ ngồi này đã được đặt trước.')
      }
  ...
  const bookTickets = async ()=>{
    try {
      if(!user) return toast.error('Vui lòng đăng nhập để tiếp tục')
        ...
        if(!selectedTime || !selectedSeats.length) return toast.error('Vui lòng chọn thời gian và chỗ ngồi.');
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      <div className='w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
        <p className='text-lg font-semibold px-6'>Thời Gian</p>
